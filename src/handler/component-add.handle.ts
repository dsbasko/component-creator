...
export class ComponentAddHandle {
	...
	verifyTemplates = async (): Promise<ComponentAddHandle> => {
		...
		if (selectedTemplate) {
			...
			for (const file of templateFiles) {
				const newFilePath = join(templatePath, selectedTemplate, file.path);
				const buffer = await this.fileService.getFileBufferToString(
					join(extensionTemplatesPath, selectedTemplate, file.path)
				);
	...
	setTemplateDirectory = async (): Promise<ComponentAddHandle> => {
		...
		this.templateFiles = templateFiles.map(({ path, isDirectory }) => ({
			fileName: basename(path),
			path: join(path),
			templatePath: join(this.templatePath, this.selectedTemplate, path),
			buffer: "",
			isDirectory,
		}));
	...
	replaceTemplateMask = async (): Promise<ComponentAddHandle> => {
		let i = 0;
		for (const file of this.templateFiles) {
			const newPath = this.stringService.replace(
				file.path,
				this.componentName
			);
			const newBuffer = this.stringService.replace(
				file.buffer,
				this.componentName
			);
	...
	writeFile = async (): Promise<ComponentAddHandle> => {
		const files = this.templateFiles
			.filter((file) => !file.fileName.includes("{{concat}}"))
		...
		if (existedFiles.length && existedFilesAction === "stop") {
			return this;
		}
		for (const file of files) {
			if (!this.contextPath) {
	...
	makeDirectory = async (): Promise<ComponentAddHandle> => {
		const files = this.templateFiles
			.filter((file) => !file.fileName.includes("{{concat}}"))
			.filter((file) => file.isDirectory);
		for (const file of files) {
			...
			const path = join(this.contextPath, file.path);
			if (!(await this.fileService.isExistPath(path))) {
				await this.fileService.makeDirectory(path);
			}
		}
		return this;
	};
	...
	concatFiles = async (): Promise<ComponentAddHandle> => {
		const files = this.templateFiles.filter((file) =>
			file.fileName.includes("{{concat}}")
		);
		for (const file of files) {
			if (!this.contextPath) {
				break;
			...
			if (oldBuffer.includes(file.buffer)) {
				continue;
			}
